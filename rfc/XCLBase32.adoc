...
which starts at the highest bits and includes the length of bytes
and the byte length of any training zeros are not important. As such encoding is aligned to the highest bits.
...
If the romanized character would be more than 2 letters, the first and last letters is taken.
